...
for dataset in DATASETS:
    ...
    model = ConvVariationalAutoEncoder(
        in_channels=1,
        in_features=data_module.sequence_length,
        latent_dim=32
    )
    ...
    torch.save(
        model.state_dict(),
        os.path.join(model_weight_dir, 'conv_variational_autoencoder.pt')
    )
    ...
    metrics.to_csv(
        './ucr_conv_variational_autoencoder.metrics.csv',
        index=False
    )
